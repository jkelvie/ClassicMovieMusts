...
module.exports = class RSS {
    ...
    async fromURL (rssUrlString) {
        console.time("INSTRUMENT RSS LOAD");
        ...
        return new Promise((resolve) => {
            const httpCallback = (response) => {
                ...
                response.on("end", () => {
                    ...
                    console.timeEnd("INSTRUMENT RSS LOAD");
                    resolve(result);
                });
            };
    ...
    fromXML (xml) {
        ...
        // Only include episodes with a movie name
        results = results.filter(episode => episode.movie );
        return { 
            podcasts: results,
        };
    }
