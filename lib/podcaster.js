...
class Podcaster {
    ...
    static instance() {
        ...
        Podcaster.singleton = new Podcaster();
        return Podcaster.singleton;
    }
...
const PlayHandler = {
    ...
    async handle(handlerInput) {
        ...
        const movieRequested = _.get(handlerInput, "requestEnvelope.request.intent.slots.MovieName.value");
        const response = handlerInput.responseBuilder;
    
        let podcast;
        // Find the podcast associated with the user's request
        if (movieRequested) {
            console.log("PlayIntent MovieName: " + movieRequested.toLowerCase());
            podcast = podcaster.feed.podcasts.find((podcast) => {
                return podcast.movie.toLowerCase() === movieRequested.toLowerCase();
            }); 
        } 

        // If there is no match, ask them again
        if (!podcast) {
            console.log("NOMATCH PlayIntent MovieName: " + movieRequested);
